...
yTrain <- sin(xTrain)
yTrain <- yTrain + rnorm(length(y), mean=0, sd=0.3)
# Validation set
...
plot(rmseTrain, type="o", pch=1, col="blue", lwd=3, xlim=c(0.8,10.2), ylim=c(-0.01,0.55),
     xlab="polynomial degree (model complexity)", ylab="root mean squared errror (rmse)")
points(rmseVal, type="o", pch=1, col="red", lwd=3)
abline(h=0, col="grey", lwd=3, lty=2)
text(9, 0.13, "training", col="blue", cex=1.5)
text(9, 0.28, "validation", col="red", cex=1.5)
